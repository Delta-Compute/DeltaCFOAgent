-- Fix Pattern Trigger to Include Justification Field
--
-- Issue: The trigger only processes entity/category/subcategory changes,
--        but ignores 'justification' changes which is where most user
--        classifications happen (86 out of 91 recent changes)
--
-- Solution: Remove the field filter or include 'justification' in the list

BEGIN;

-- Drop the old trigger (keep the function since we're just modifying the condition)
DROP TRIGGER IF EXISTS trigger_check_pattern_suggestion ON user_classification_tracking;

-- Recreate the trigger function with justification included
...
BEGIN
    -- FIXED: Now includes 'justification' field
    -- Only process if entity, category, subcategory, OR justification was changed
    IF NEW.field_changed NOT IN ('entity', 'category', 'accounting_category', 'subcategory', 'justification') THEN
        RETURN NEW;
    END IF;
    ...
    IF v_similar_count >= 3 THEN
        ...
        RAISE NOTICE 'Pattern suggestion created: ID=% count=% pattern=%', v_new_suggestion_id, v_similar_count, v_description_pattern;
...
'Automatically creates pattern suggestions including justification field changes';
