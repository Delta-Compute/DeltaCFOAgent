#!/usr/bin/env python3
"""
Reset ONE approved pattern suggestion to pending for testing
"""
import sys
import os
...
with db_manager.get_connection() as conn:
    ...
    print('=' * 70)
    print('RESETTING ONE APPROVED PATTERN TO PENDING STATUS FOR TESTING')
    print('=' * 70)

    # Reset pattern ID 2 (the one with high confidence that was previously approved)
    cursor.execute("""
        ...
        WHERE tenant_id = 'delta'
          AND id = 2
        RETURNING id, description_pattern, status
    """)
    ...
    if result:
        ...
        print(f'  New Status: {result[2]}')
    else:
        print('\nNo pattern found with ID 2')
