...
def upload_file():
    ...
    try:
        ...
        if file_ext == '.pdf':
            ...
            transactions_found = pdf_result.get('total_found', 0)
            if transactions_found == 0:
                ...
                return jsonify({
                    'success': False,
                    'error': 'No transactions found in PDF. Please ensure the PDF contains transaction data (bank statement, invoice, receipt, etc.)'
                }), 400

            print(f"✅ PDF processing complete: {transactions_found} transactions extracted")

            # Clean up uploaded file (we've extracted the data)
            if os.path.exists(filepath):
                os.remove(filepath)
            ...
            return jsonify({
                'success': True,
                'message': f'Successfully extracted {transactions_found} transaction(s) from PDF',
                'transactions_processed': transactions_found,
                'document_type': pdf_result.get('document_type', 'Unknown'),
                'transactions': pdf_result.get('transactions', [])
            })

        # CSV processing below - create backup first
